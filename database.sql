...
CREATE TABLE IF NOT EXISTS `sisane`.`paciente` (
  ...
  PRIMARY KEY (`id`),
  INDEX `fk_paciente_servicio1_idx` (`servicio_id` ASC),
  CONSTRAINT `fk_paciente_servicio1`
...
CREATE TABLE IF NOT EXISTS `sisane`.`analitica` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `informe` LONGTEXT NULL,
  `fecha_petición` DATETIME NULL,
  `importe` DOUBLE NULL,
  `color_id` INT NULL,
  `anticoagulante_id` INT NULL,
  `tipomuestra_id` INT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_analitica_color1_idx` (`color_id` ASC),
  INDEX `fk_analitica_anticoagulante1_idx` (`anticoagulante_id` ASC),
  INDEX `fk_analitica_tipomuestra1_idx` (`tipomuestra_id` ASC),
  CONSTRAINT `fk_analitica_color`
    FOREIGN KEY (`color_id`)
    REFERENCES `sisane`.`prioridad` (`id`)
    ON DELETE SET NULL
    ON UPDATE CASCADE,
  CONSTRAINT `fk_analitica_anticoagulante`
    FOREIGN KEY (`anticoagulante_id`)
    REFERENCES `sisane`.`anticoagulante` (`id`)
    ON DELETE SET NULL
    ON UPDATE CASCADE,
  CONSTRAINT `fk_analitica_tipomuestra`
    FOREIGN KEY (`tipomuestra_id`)
